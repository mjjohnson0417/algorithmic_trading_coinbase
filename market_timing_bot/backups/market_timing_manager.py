...
class MarketTimingManager:
    ...
    def identify_key_levels(self, symbol: str) -> Dict[str, Any]:
        ...
        # Use cached indicators directly
        try:
            with self.cache_lock:
                indicators = self.indicators_cache.get(symbol, {})
            levels = []
            ...
            if '6h' in indicators:
                self.logger.debug(f"Processing levels for {symbol} (6h)")
                # Pivot Points
                pivot_points = indicators['6h'].get('pivot_points', {})
                for name in ['s1', 'r1']:
                    if name in pivot_points and pivot_points[name] > 0:
                        try:
                            price = float(pivot_points[name])
                            is_support = name == 's1'
                            levels.append((price, f'pivot_{name}_6h', is_support))
                        except (ValueError, TypeError):
                            self.logger.warning(f"Invalid pivot point {name} for {symbol} (6h), skipping")
                # ... (rest of the method remains the same)
            
            # Proximity check (0.25%)
    ...
    def determine_trade_levels(self, symbol: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # Print input dictionaries
            self.logger.debug(f"Inputs for {symbol}:")
            self.logger.debug(f"market_data: {json.dumps(market_data, default=str)}")
            ...
            # Get all S/R levels from indicators_cache
            ...
            for level in levels.get('nearby_levels', []):
                ...
                if (is_support or is_support is None) and (not apply_trend_filter or trend != 'downtrend'):
                    if current_price <= price:  # Buy price not above current price
                        next_resistance = next((p for p, _, _ in sr_levels if p > price), None)
                        if next_resistance and (next_resistance - current_price) / current_price >= 0.015:
                            if self._has_bullish_confirmations(symbol, ['1h', '15m', '5m'], market_data, chart_patterns, candlestick_patterns, direction_evaluation):
                                ...
                                if rr_ratio >= 3.0:  # 1:3 risk-reward
                                    trades.append({
                                        'direction': 'long',
                ...
                if (is_support is False or is_support is None) and (not apply_trend_filter or trend != 'uptrend'):
                    if current_price >= price:  # Sell price not below current price
            ...
            self.logger.debug(f"Trades for {symbol}: {trades}")
            return trades
        except Exception as e:
